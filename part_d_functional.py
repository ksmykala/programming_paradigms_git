...
def get_total_functional(expenses):
    return sum(expense["amount"] for expense in expenses)
def get_category_totals_functional(expense):
    return{
        cat: sum(expense["amount"] for expense in expenses if expense["category"] == cat)
        for cat in {expense["category"] for expense in expenses}
    }
def get_above_average_functional(expenses):
    avg = sum(expense["amount"] for expense in expenses) / len(expenses)
    return list(filter(lambda expense: expense["amount"] > avg, expenses))
