...
for expense in expenses:
    if expense["amount"] > average_expense:
        # Save each entry in format: Catergory - Amount
        # then add to the back of the list
        entry = f"{expense["description"]} ({expense["category"]}) - {expense["amount"]:.2f}"
        above_average_list.append(entry)

print("Expenses above average:")
for item in above_average_list:
    print(f" - {item}")
